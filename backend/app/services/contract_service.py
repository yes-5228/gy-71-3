...
def validate_contract_termination(db: Session, contract: Contract) -> tuple[bool, str]:
    if contract.status != "active":
        return False, "只有履行中的合同可以终止"
    unpaid_count = _get_unpaid_payment_count(db, contract.id)
    if unpaid_count > 0:
        return False, f"存在 {unpaid_count} 笔未结清账单，请先处理完毕后再终止合同"
    if _get_deposit_unhandled(contract):
        return False, "存在未处理押金，请先退还押金后再终止合同"
    return True, ""
...
def enrich_contract_termination_flag(db: Session, contracts: list[Contract]) -> list[Contract]:
    for contract in contracts:
        if contract.status == "active":
            can_terminate, reason = validate_contract_termination(db, contract)
...
def update_contract(db: Session, contract_id: int, payload: ContractUpdate) -> Contract | None:
    ...
    will_terminate = update_data.get("status") == "terminated" and contract.status != "terminated"
